...
The wrapper keeps HOME pointed at the real home for Codex authentication, creates and cleans only a private runtime directory, uses `env -i`, and sets explicit native toolchain paths. Rust is installed system-wide under `/opt/rust`; writable cargo state is per execution.

Adapt the Codex permission profile to deny the complete real home, trusted installation, system-wide Rust, runner workspace root, and general temp roots before re-allowing only the selected repository, its `.git` read access, and the private runtime directory. Keep network access and never use danger-full-access.
...
- derive the source root and GitHub repository URL from the Git `origin` remote;
...
- register runner name `gh-runner`, labels `agent-relay,gh-runner`, and work directory `_work` relative to the runner installation;
- install, enable, and start the official runner service through `svc.sh`;
- preserve existing registration, authentication, workspaces, and a newer self-updated runner on rerun;
...
The source checkout location is not a runtime contract. The user may keep it under `/srv/github-runner`, but runner workspaces remain under the runner's Linux `_work` directory.

### 4. Isolate new jobs from the old runner

The old runner remains untouched and must not receive jobs intended for the new runtime.
...
The old `agent-relay`-only runner cannot match new jobs.
...
Set `CODEX_WORKSPACE_ROOT` from `${{ runner.workspace }}` rather than a host-specific path.
...
Keep repository/package/workflow names, compatibility labels, and historical completed plans. Do not perform a broad rename or unrelated refactor.

### 7. Rewrite tests

Replace removed-layer tests with deterministic coverage of:

- direct CLI inputs, `${{ runner.workspace }}` containment, plan boundaries, commit-message normalization, and plan movement;
- actual executor success, failure, spawn error, timeout, force-kill, truncation, split UTF-8, split secrets, and live redaction;
- wrapper proof that real-home, auth, runner, source, and other workspace fixtures remain unchanged;
- installer preflight, toolchain decisions, remote normalization, Codex-login branching, hidden token input, first registration, `_work`, dual labels, `svc.sh`, failure preservation, repository mismatch, and token-free rerun;
- proof that the token is absent from output, generated files, installed files, units, profiles, and child environments;
- workflow dual labels, trusted installed paths, workspace-context propagation, GitHub-token scoping, artifact upload, and finalization;
...
Documentation may recommend the source checkout under `/srv/github-runner` and may tell the user to start the prepared host with `wsl -d gh-runner`. Runtime code and installer must not depend on that path or detect WSL.
...
3. Update production and example workflows for direct installed execution and runner workspace context.
...
- [x] (2026-07-16) Corrected old-runner job collision through dual labels.
- [x] (2026-07-16) Removed unsafe temporary-auth-home and host-mounted workspace assumptions.
...
- Putting copied or linked auth inside a writable temporary HOME could expose it to model-controlled tools. The native design keeps real HOME for the Codex host process and denies the complete home in the model permission profile.
- Reusing only the `agent-relay` label would allow the untouched old runner to receive new jobs. Requiring both `agent-relay` and `gh-runner` isolates the fresh installation.
- Putting Actions workspaces below the mounted source folder is unnecessary. The official `_work` directory in the Linux runner installation is simpler and avoids a host-specific implementation path.
...
- Decision: remove Relay as a process and transport while preserving repository identity and orchestration behavior.
...
- Decision: require dual labels `agent-relay,gh-runner`.
  ...
  Date/Author: 2026-07-16 / deployment review.

- Decision: use the official Linux `_work` directory and `${{ runner.workspace }}`.
  Rationale: runtime workspaces do not need the Windows-mounted source path, and workflow context provides the exact job boundary without hardcoding a username or host path.
  Date/Author: 2026-07-16 / path and isolation review.
...
- Decision: keep real HOME for the Codex host process but deny the complete home to model-controlled tools.
  Rationale: Codex uses its current login without copying credentials, while the model cannot read auth or runner credentials.
...
3. The real launcher changes only its private temp fixture and preserves real-home, auth, runner, source, and unrelated workspace fixtures.
4. The real installer passes first-run and rerun fixture tests, including login branching, hidden token input, `_work`, dual labels, official service commands, repository mismatch, and token non-persistence.
5. Production, example, and CI workflows require both labels and the old runner cannot match them.
6. The Codex step uses the installed trusted CLI, receives `${{ runner.workspace }}`, uploads output on failure, and receives no GitHub token.
...
Rerunning `install.sh` updates required packages and the root-owned harness while preserving runner registration, Codex authentication, `_work`, and a newer runner version. Failed source validation or staging leaves the installed harness unchanged. Failed registration persists no token and is retried through the same installer.
...
- repository/package/workflow names and the `agent-relay` compatibility label.

...
The Agent Relay workflow now contains a validation gate tied to the exact PR head. The Codex job cannot start when that validation fails. The runtime invokes the managed launcher instead of a nonexistent installation path, and startup failures now include the original operating-system diagnostic.

GitHub Actions CI run `29629994575` (`CI #616`) completed successfully on commit `ecdb8fd63893992cfbed446e014af21ec474d912`. It ran dependency installation and the complete `npm run check` validation successfully.

The implementation also corrected a pre-existing false failure in the update-system harness by preventing a redundant nested unit-test run after fixture-specific host-path rewriting. The full unit-test suite remains executed by the outer repository validation.

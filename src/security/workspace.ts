...
export async function resolveWorkspace(root: string, requestedPath: string): Promise<string> {
  ...
  if (!info.isDirectory()) {
    throw new CodexExecutionError("WORKSPACE_NOT_FOUND", "GITHUB_WORKSPACE is not a directory");
  }
...
export async function assertActivePlanFile(workspace: string, planPath: string): Promise<string> {
  ...
  if (!relativePlan || relativePlan.includes(sep) || !relativePlan.endsWith(".md") || isOutside(activeRoot, candidate)) {
  ...
  if (resolvedCandidate !== candidate) {
    throw new CodexExecutionError("INVALID_PLAN", "Active ExecPlan must not traverse symbolic links");
  }

  const info = await stat(resolvedCandidate);
  if (!info.isFile()) {
    throw new CodexExecutionError("INVALID_PLAN", "Active ExecPlan must be a regular file");
  }

...
`ansible/playbooks/host.yml` provisions and updates the full host without a GitHub PAT. It manages packages, users, Docker, toolchains, runner binaries, the systemd unit, source checkout, and Agent Relay runtime. On an unregistered host it leaves the runner listener disabled and stopped.
...
Shared runner, toolchain, and host constants are defined in [`config/runner-host.json`](config/runner-host.json). Codex uses `/srv/github-runner/storage/docker-socket/docker.sock`; `/run/docker.sock` remains available for host operators. Consumer workflows target `[self-hosted, agent-relay]` and retain the exact `runner.name == gh-runner` assertion as defense in depth.

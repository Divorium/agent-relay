...
This is not a migration. The implementation must not inspect, stop, copy from, clean, or modify the previous Docker deployment. Existing Docker runner state, volumes, Relay state, logs, and Codex authentication are not inputs.
...
The prepared host provides Debian, systemd, one non-root user with sudo, a writable `/srv/github-runner`, and outbound network access.

Repository code must not detect or configure WSL, Windows, drive mounts, virtual-disk limits, or host lifecycle.
...
The script must not ask for repository URL, runner name, labels, workspace, Relay credentials, Codex auth path, UID, GID, service values, or an environment file.

## Separate the new runner from the old runner

Register the new runner with both custom labels:

- `agent-relay`;
- `gh-runner`.

Production and CI workflows must require:

```yaml
runs-on: [self-hosted, agent-relay, gh-runner]
```

The old Docker runner has only `agent-relay`, so it cannot receive jobs requiring both labels. The new installation does not need to stop, unregister, or change the old runner.
...
- launcher: `/usr/local/bin/codex-run`;
- runner directory: `$HOME/.local/share/actions-runner`;
- work root: `/srv/github-runner/_work`;
...
No credentials or persistent state are stored in the checkout or work root.
...
Update `scripts/toolchain-smoke.sh` for these native requirements. It must not reject unrelated software such as OpenSSH or .NET.

Before installing the harness, run:
...
Stage the runtime payload and replace `/opt/agent-relay` only after validation succeeds. Install the compiled direct CLI, its imported modules, `runner/resolve-pr.mjs`, `runner/finalize.sh`, and `/usr/local/bin/codex-run`. Production code uses Node built-ins and requires no runtime `node_modules`.
...
4. Run `config.sh --unattended --replace` with the derived repository URL, name `gh-runner`, labels `agent-relay,gh-runner`, and work directory `/srv/github-runner/_work`.
5. Unset the token immediately.
6. Install and start the official service through `svc.sh` for the current user.

On rerun, preserve runner-managed files and Codex auth, do not request a token when `.runner` exists, do not downgrade the runner, validate that the registration targets the repository derived from `origin`, and ensure the official service is enabled and active.
...
Replace `src/server.ts` with a strict CLI compiled under `/opt/agent-relay`. Workflows invoke the installed CLI, not a direct-execution file from the pull-request checkout.
...
Remove request IDs, HTTP calls, job IDs, polling, result DTOs, persisted Relay logs, and restart state. Replace API-specific errors with a minimal execution error only where needed.
...
Do not copy, move, rewrite, or symlink auth. Do not modify the real home, runner directory, source checkout, or work root from the wrapper.
...
Use a neutral Codex permissions profile such as `agent`. It must:

- disable memories;
- deny the complete real user home;
- deny `/opt/agent-relay` and `/opt/rust`;
- deny the complete work root before re-allowing the selected repository;
...
The deny rules constrain model-controlled tools. The Codex host process still receives the real HOME and can use its auth, matching the current design in which the model cannot read the credential directory.
...
All must require `[self-hosted, agent-relay, gh-runner]`.

The Codex step invokes the installed direct CLI with `pipefail` and `tee` into `${RUNNER_TEMP}/codex-console.log`. Upload the artifact with `if: always()`, then fail when Codex failed. Finalization uses the installed `finalize.sh`, the derived message, API-derived branch, and step-scoped `${{ github.token }}`.
...
Keep historical completed ExecPlans unchanged.
...
- direct CLI validation, workspace/plan boundaries, integer parsing, and commit-message normalization;
...
- launcher proof that real-home, auth, runner, and source fixtures are unchanged;
- installer preflight, toolchain decisions, remote normalization, login branching, hidden token input, first registration, dual labels, `svc.sh`, failure preservation, repository mismatch, and token-free rerun;
- proof that the token is absent from output, files, units, profiles, and child environments;
- workflow dual labels, trusted installed paths, token scoping, artifact upload, and finalization;
...
It may tell the user to start the prepared host with `wsl -d gh-runner`; code and installer must not call or detect WSL.

Acceptance requires `npm ci` and `npm run check`, deterministic execution of the real CLI/launcher/installer/resolver/finalizer through fixtures, dual-label workflow isolation from the old runner, no Relay transport or secret, no access from model-controlled tools to the user home, idempotent installer behavior, no old-environment operations, and no broad unrelated refactor.
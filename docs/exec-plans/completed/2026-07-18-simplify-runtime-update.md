...
The pipeline runs `npm ci` followed by explicit phases for type checking, tests and coverage, production-runtime compilation, shell syntax, Node syntax, toolchain smoke, and system integration tests.
...
The `CI` workflow uses `${{ github.ref }}` as its concurrency group with `cancel-in-progress: true`, so a newer run for the same pull-request ref supersedes older queued or running CI work. Failed test output is retained as an artifact. The agent-execution workflow is exposed in GitHub Actions as `Codex`; its existing per-PR serialization remains unchanged. The example workflow is kept byte-for-byte aligned with the production Codex workflow.
...
- [x] Validate the complete repository in the self-hosted GitHub Actions pipeline.
- [x] Review the implementation for environment-specific failures and update this plan.
- [x] Record final CI outcomes and move this plan to `docs/exec-plans/completed/`.
...
- The GitHub Actions workflow that invokes the agent is named `Codex`, and its example remains synchronized.
...
A workflow review added ref-scoped CI concurrency to prevent repeated commits from accumulating stale self-hosted jobs and renamed the agent workflow from `Agent Relay` to `Codex`. The Codex workflow's separate per-PR concurrency policy was intentionally left unchanged. The new CI configuration cancelled stale run #650.

The first full CI review then exposed a stale example workflow name. The production workflow had been renamed to `Codex`, while `examples/github-actions/agent-relay.yml` still used `Agent Relay`; the strict equality regression test correctly failed. The example was synchronized with production, and CI #672 passed every explicit validation phase.
...
11. The production and example Codex workflows remain identical.
...
- Renaming only the production workflow was insufficient because a regression test deliberately requires the checked-in example to be identical.
...
The simplified updater now follows the requested direct replacement model and no longer contains the ownership-sensitive staged-runtime transaction that caused the production failure. CI #672 passed type checking, tests with 100% runtime coverage, production runtime compilation, shell and Node syntax validation, real toolchain smoke, and both system integration harnesses. The PR remains draft for operator review.